...
```{r deduplicate_pid,lab_id_sampletype}
...
# plot bar chart
sample_by_month %>% 
  ggplot(aes(fct_infreq(sample),fill = sample)) + 
  geom_bar(stat = "count") +
  labs(x = "", y = "") +
  coord_flip() + 
  facet_wrap(~collection_date_in_month,ncol = 3) +
  theme_bw() +
  theme(legend.position = "NULL",axis.text = element_text(color = "black"),
        axis.title = element_text(size = 10),
        panel.grid.major = element_blank()) +
  geom_text(stat = 'count', aes(label = ..count..), size = 2.4, hjust = -0.1) +
...
dedup_by_id_stype %>% 
  ...
  geom_text(stat = 'count', aes(label = ..count..), size = 2.4, hjust = -0.1) +
  scale_y_continuous(limits = c(0,ymax$n + 25),expand = c(0.01, 0.1)) +
  facet_wrap(~fct_infreq(sample_source),scales = "free_x",ncol = 4) +
  theme_bw() +
  theme(legend.position = "non",
        axis.text = element_text(color = "black"),
        axis.title = element_text(face = "italic",hjust = 0.95, color = "red"),
        panel.grid.major = element_blank(),
        strip.background = element_rect(fill = "#CEB5CE"),
        strip.text = element_text(size = 10))
  
rm(dedup_by_id_stype)
rm(dedup_by_pid)
rm(ymax)
```
...
![](Images/True_pathogen_formula.png){width="50%"}
...
bc_first_isolate %>% 
  ggplot(aes(fct_infreq(result), fill = result)) +
  geom_bar() +
  ...
  theme(axis.text.y = element_text(face = "italic", color = "black"),
        ...
        axis.text = element_blank(),
        axis.ticks.y.right = element_blank()) +
  scale_y_continuous(limits = c(0,ymax$n + 3),expand = c(0.01, 0.1))
...
![](Images/Contamination_formula.png "contamination"){width="343"}
</center>
...
- Cefoxitin disk for methicillin-resistant staphylococcus aureus (MRSA)  
- Vancomycin Etest for vancomycin intermediate (VISA) or vancomycin resistant (VRSA)  
