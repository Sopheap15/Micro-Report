...
```{r ward count}
ward_count <- ifelse(data %>% distinct(sample_source) %>% count() >= 25, 16, 8)
```
...
### 5.6. Blood culture day of positive
Microbiology laboratory always reports day of blood culture positive in comment. We filtered common organism isolated. `r comment %>% filter(is.na(day_growth)) %>% count()` record(s) were not mentioned day of positive and excluded from analysis.
