...
We assume a patient receives care at a hospital (inpatients and outpatients) or private clinics and has only one patient identification number. We removed duplicates to find the actual number of patient requests. Thus, we have: **`r dedup_by_pid %>% count() %>% mutate_if(is.numeric,format,big.mark=",")`** patient(s).
...
dedup_by_pid %>%
  ...
  geom_text(aes(label = paste(pct,"%"), x = 1),
...
Distribution of age group and gender. We accepted the range 0 to 110 years old. We have **`r dedup_by_pid %>% filter(age>=0, age<=110) %>% count() %>% mutate_if(is.numeric,format, big.mark = ",")`** patient(s)
...
#dedup_by_pid %>% 
  ...
  # plot_ly(type = "bar", x = ~age, y = ~ M, name = "Male", 
  #         hoverinfo = "text",
  #         hovertext = ~paste(" Age:",age,"year\n", "Male: n=",M),
  #         marker = list(color = "rgb(3, 191, 196)")) %>% 
  # add_trace(y = ~ M, name = "Female",
  #           hoverinfo = "text",
  #           hovertext = ~paste(" Age:",age,"year\n","Female: n=", F),
...
A patient can have more than one microbiology specimen collected. We assume one specimen has one unique sample identification number. After removing duplicates, we have: **`r dedup_by_id_stype %>% count() %>% mutate_if(is.numeric,format,big.mark=",")`** sample(s). We stratify data by month and sample type
...
sample_by_month %>% 
  ...
  scale_y_continuous(limits = c(0, ymax$n + 25),
                     expand = c(0.01, 0.1)) +
  scale_fill_manual(values = c("Blood Culture" = "#F7766D", # blood red
...
```{r ward count}
ward_count <- ifelse(data %>% distinct(sample_source) %>% count() >= 25, 17, 12)
```

```{r microbiology sample by ward, fig.width=8, fig.height= ward_count}
...
bc_first_isolate %>%
  ...
  theme(axis.text.y = element_text(face = "italic", color = "black"),
        ...
        axis.ticks.y.right = element_blank()) +
  scale_y_continuous(limits = c(0, ymax$n + 7),expand = c(0.01, 0.1))
...
### 5.4. Blood culture true pathogen rate and contamination rate by wards
...
ad_volume %>% 
  ggplot(aes(collection_date_in_month, pect, 
             fill = factor(vs,levels = c("Too low (<8ml)","Correct (8-12ml)","Too high       
                                         (>12ml)")))) +
...
adult_request <- data %>%
  filter(sample == "Blood Culture", age >= 14) %>% 
...
solitary_bottle <- data %>%
  filter(sample == "Blood Culture", age >= 14, !is.na(volume1)) %>% 
  distinct(lab_id, .keep_all = T) %>% 
...
ggplot() +
  ...
  scale_y_continuous(sec.axis = sec_axis(~.*2, name = "Solitary bottle")) +
  labs(x = "Month",
       y = "Adult patient",
...
There were `r miss_bv` bottle(s) with empty blood culture volume field in CamLIS. This table is used to monitor number of blood culture collected per patient and impact of missing blood culture volume data. 
...
Microbiology laboratories always report the day of blood culture positivity in the patient report comment. We present the common pathogens isolated. `r comment %>% filter(is.na(day_growth)) %>% count()` patient(s) had no record of the day of positivity and therefore could not be included in the analysis.

```{r day of positive}
organism <- c("Escherichia coli", "Klebsiella pneumoniae", "Acinetobacter","Burkholderia pseudomallei", "Pseudomonas aeruginosa", "Non-fermenting gram negative rods", "Staphylococcus aureus", "Coagulase Negative Staphylococcus")

day_growth <- comment %>% 
  filter(result %in% organism, !is.na(day_growth)) %>% 
  mutate(result = factor(result, levels = organism)) %>% 
  group_by(result, day_growth) %>% 
  count(day_growth)

day_growth %>% 
  ggplot(aes(day_growth, n, fill = day_growth)) + 
  geom_bar(stat = "identity", show.legend = F, width = 0.7) +
  geom_text(aes(label = n), vjust = -0.1, size = 2.7) +
  facet_wrap(~ result) +
  labs(y = "Cases", x = "Day") +
  coord_cartesian(expand = c(0, 0), ylim = c(0, max(day_growth$n) * 1.3 )) + 
  theme_bw() +
  theme(
        axis.text = element_text(color = "black"),
...
**Tracking [blood culture]{style="color:red"} isolates by month for possible Healthcare Associated Infection (HAI)**
