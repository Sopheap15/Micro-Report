...
```{r ward count}
ward_count <- ifelse(data %>% distinct(sample_source) %>% count() >= 25, 17, 8)
```
...
bc_first_isolate %>%
  mutate(desc = case_when(age < 14 ~ "Pediatric (<14 years old)",
                          TRUE ~ "Adult (>=14 years old)"),
         result = recode(result,"Streptococcus, beta-haemolytic (not Group A, not Group B)" = "Beta-streptococcus non A, non B")) %>% 
  group_by(result, desc) %>% 
  mutate(n = n()) %>% 
  group_by(result) %>% 
  mutate(m = n()) %>%
...
adult_request <- data %>%
  ...
  distinct(patient_id, lab_id, .keep_all = T) %>% # deduplicate by Pid, Lid
  group_by(collection_date_in_month) %>%
...
ad_sol %>% 
ggplot() +
  geom_bar(aes(collection_date_in_month, ad_request, fill = collection_date_in_month), show.legend = F, stat = "identity", width = 0.5) +
...
day_growth %>% 
  ...
  facet_wrap(~ result) +
  labs(y = "Cases", x = "Day") +
  coord_cartesian(expand = c(0, 0), ylim = c(0, max(day_growth$n) * 1.2 )) + 
  theme_bw() +
  theme(
        axis.text = element_text(color = "black"),
        #panel.grid.major = element_blank(),
...
da %>% 
  ...
  scale_y_continuous(limits = c(0, max(da$total) + 2), breaks = seq(0, max(da$total) + 2, 1)) +
  theme(legend.title = element_blank()) +
  #guides(fill = guide_legend(reverse=TRUE))+
  scale_fill_manual(values = c("white",# blank
                             "#f7766d", # blood pink
