...
reject_com <- reject_spe %>% # change word to no sample or no label
  mutate_at(vars(comment, reject_comment), ~str_replace_all(., pattern = "\u200B","")) %>% 
  mutate_at(vars(comment, reject_comment), ~ifelse(str_detect(., no_sample) == TRUE,
                                                     "no sample", .)) %>%
  mutate_at(vars(comment, reject_comment), ~ifelse(str_detect(., no_label) == TRUE,
                                                     "no label", .))
...
org <- data %>% # from data set select only notifiable organism
  ...
  filter(results %in% c(org_notifiable)) %>%
  arrange(collection_date_in_month) %>% 
  distinct(patient_id, .keep_all = T) %>% 
...
sal <- data %>% 
    ...
    filter(results %in% c("Salmonella Paratyphi A","Salmonella Typhi", "Salmonella")) %>% 
    summarise(n = n())
