...
p <- csf %>%   
  ...
  facet_wrap(~desc) +
  geom_text(aes(label = n, y = n + (mean(n)*0.08)), size = 3, hjust = -0.2) +
  coord_flip() +
  labs(x = "", y = "Cases") +
  theme_bw() +
...
p <- blood_path %>%
  ...
  facet_wrap(~ desc) +
  geom_text(aes(label = n, y = n + (mean(n)*0.30)), size = 3) +
  labs(x = "",y = "Cases") +
  coord_flip() +
  theme_bw() +
  theme(axis.text.y = element_text(face = "italic", color = "black"),
...
request <- data %>% 
  filter(sample == "Blood Culture") %>% 
  distinct(patient_id, sample,.keep_all = T) %>% 
  group_by(sample_source) %>% 
...
blank <- cbind(month, 'Bottle A' = rep(0, 12), 'Bottle B' = rep(0, 12), Total = rep(0,12))
  ...
  bottle_A <- bottle %>% 
    filter(!is.na(result),is.na(volume1)) %>% 
    summarise('Bottle A' = n()) 
  
  bottle_B <- bottle %>% 
    filter(!is.na(result), is.na(volume1), is.na(volume2),age >= 5110) %>%
...
da <- rbind(blank_data, d) %>% 
 ...
 p <- da %>%
  ...
  geom_bar(aes(month,n, fill = sample, 
   ...
   geom_text(aes(month, total + 0.15 , label = paste0("Total: ", total)), size = 3,
                stat = "identity") +
  theme_bw() +
...
S <- binconf(da$S, da$Total) %>% 
  as.data.frame() %>% 
  round(1) %>% 
  mutate(S = paste0("(",PointEst*100,"%)[",Lower*100,"-", Upper*100,"]"))
I <- binconf(da$I, da$Total) %>% 
  as.data.frame() %>% 
  round(1) %>% 
  mutate(I = paste0("(",PointEst*100,"%)[",Lower*100,"-", Upper*100,"]"))
R <- binconf(da$R, da$Total) %>% 
  as.data.frame() %>% 
  round(1) %>% 
  mutate(R = paste0("(",PointEst*100,"%)[",Lower*100,"-", Upper*100,"]"))

cbind(da, S$S, I$I, R$R) %>%
  mutate(Antibiotic = ab_name(Antibiotic),
         "S (%)[95CI]" = paste(S, `S$S`),
         "I (%)[95CI]" = paste(I , `I$I`),
         "R (%)[95CI]" = paste(R , `R$R`)
         ) %>% 
  select(Antibiotic, `S (%)[95CI]`, `I (%)[95CI]`, `R (%)[95CI]`, Total) %>% 
    kable(align = c("l",'l', 'l','l','c'), table.attr = "style='width:100%;'") %>% 
  kable_styling(bootstrap_options = c("condensed","striped", "hover","responsive")) %>% 
...
There were [`r sau` *Staphylococcus aureus*]{style="color:red;font-weight:bold;"} reported.
```{r S.aureus cAST}
if (sau > 0) {
  da <- bc_first_isolate %>% 
