...
reject_spe <- reject_spe %>% 
  mutate(reject = case_when(sample == "Sputum" ~ str_extract(comment, ".*>.*10.*epi.*LPF")),
         rejected_comment = case_when(
           ...
           ), #មិនមាន វិភាគ https://www.branah.com/unicode-converter
...
p <- csf %>%   
  ...
  geom_bar(stat = "identity", width = 0.2, aes(text = paste0(result,": ",n))) + 
  facet_wrap(~desc) +
...
Positive blood cultures are life threatening for patients and require immediate reporting. We analysed data and found [`r critical_result$p`% (n = `r critical_result$total`)]{style="color:blue; font-size:16px; font-weight:bold; background-color: #FFEB14"} of critical results were recorded in CamLIS as reported.
