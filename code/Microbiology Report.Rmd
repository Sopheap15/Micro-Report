...
p <- sample_by_month %>% 
  ...
  geom_text(aes(label = n, y = n + 7), size = 2.5) +
  labs(x = "", y = "") +
  coord_flip() + 
...
```{r top12, fig.width=8, fig.height= ward_count}
...
p <- dedup_by_id_stype %>% 
  ...
  ggplot(aes(reorder(sample, n), n, fill = sample)) + 
  geom_bar(stat = "identity", aes(text = paste0(sample,": ",n))) + 
  geom_text(aes(label = n, y = n + 4), size = 2.4) +
  coord_flip() +
...
reject_spe <- reject_spe %>% 
  mutate(reject = case_when(sample == "Sputum" ~ str_extract(comment, ".*>.*10.*epi.*LPF")),
         rejected_comment = case_when(
           str_detect(rejected_comment,
                      pattern = "\u1798\u17b7\u1793\u1798\u17b6\u1793(.*)?\u179c\u17b7\u1797\u17b6\u1782(.*)?") == TRUE ~ "No Sample",
           TRUE ~ rejected_comment), #មិនមាន វិភាគ https://www.branah.com/unicode-converter
         rejected_comment = str_to_title(rejected_comment)
         ) %>% 
  mutate(reject = coalesce(reject, rejected_comment)) %>% 
  filter(!is.na(reject)) %>% 
  group_by(sample, reject) %>% count() 

left_join(reject_spe, spe) %>% 
  select(Specimen = sample, `Total specimen` = m, `Total reject` = n, Reasons = reject) %>% 
...
p <- csf %>%   
  ...
  geom_bar(stat = "identity", width = 0.3, aes(text = paste0(result,": ",n))) + 
  facet_wrap(~desc) +
  geom_text(aes(label = n, y = n + .3), size = 3, hjust = -0.2) +
  coord_flip() +
  labs(x = "", y = "Cases") +
  theme_bw() +
...
<center>![formula calculate true pathogen](Images/True_pathogen_formula.png){width="50%"}</center>
...
p <- blood_path %>%
  ...
  facet_wrap(~ desc) +
  geom_text(aes(label = n, y = n + 0.4), size = 3) +
  labs(x = "",y = "Cases") +
  coord_flip() +
  theme_bw() +
  theme(axis.text.y = element_text(face = "italic", color = "black"),
...
<center>![Contamination_formula](Images/Contamination_formula.png "contamination"){width="343"}</center>
...
p <- day_growth %>% 
  ...
  geom_text(aes(label = n, y = n + 0.3), size = 2.7) +
  facet_wrap(~ result) +
  labs(y = "Cases", x = "Day") +
  scale_y_continuous(expand = expansion(mult = c(.01, .10))) +
  theme_bw() +
...
TAT %>% 
  ...
  plot_ly() %>% 
  add_trace(y = ~tat_h, type = "box", 
            boxpoints = "all",
            jitter = 0.7,
            pointpos = 0,
            name = "Collection-Primary report", 
            boxmean = TRUE,
            color = "white",
            marker = list(symbol = "dot", color = "black"),
            hoverinfo = "y") %>% 
...
Positive blood cultures are life threatening for patients and require immediate reporting. We analysed data and found [`r critical_result$p`%]{style="color:blue; font-size:20px; font-weight:bold; background-color: #FFEB14"} of critical results were recorded in CamLIS as reported.     
...
da <- rbind(blank_data, d) %>% 
 ...
 p <- da %>%
  ...
  geom_bar(aes(month,n, fill = sample, 
   ...
   geom_text(aes(month, total + 0.2 , label = paste0("Total: ", total)), size = 3,
               stat = "identity") +
  theme_bw() +
...
```{r bps AMR,results='asis', eval=FALSE}
if (bps > 0) {
data %>% 
  filter(mo == "B_BRKHL_PSDM") %>% 
  select(mo,AMC,CAZ,MEM,TMP,GEN) %>% 
