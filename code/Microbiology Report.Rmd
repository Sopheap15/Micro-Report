...
rstring <- "\u1798\u17b7\u1793\u1798\u17b6\u1793|\u1782\u17d2\u1798\u17b6\u1793|\u1796\u17bb\u17c6\u1798\u17b6\u1793(.*)?\u179c\u17b7\u1797\u17b6\u1782(.*)?"  #[មិន, គ្មាន] វិភាគ https://www.branah.com/unicode-convert

spe <- dedup_by_id_stype %>% 
  #distinct(patient_id, lab_id, .keep_all = T) %>% 
  group_by(sample) %>% summarise(m = n())


reject_com <- reject_spe %>% 
  mutate(comment = ifelse(str_detect(comment, rstring) == TRUE, 
                          "No Sample", comment),
         reject = str_extract(comment, ".*>.*10.*epi.*LPF|No Sample"),
         rejected_comment = ifelse(str_detect(rejected_comment, rstring) == TRUE,
                                   "No Sample", rejected_comment),
          rejected_comment = str_replace(rejected_comment, "Sugg.*$","")
         # rejected_comment = str_extract(rejected_comment, ".*>.*10.*epi.*LPF|No Sample")
         ) %>% 
...
rm(rstring, spe, reject_com, total_spe)
```
...
We analysed data and found [`r critical_result$p`%]{style="color:blue; font-size:16px; font-weight:bold; background-color: #FFEB14"} (`r critical_result$total` patient(s)) of critical results were recorded in CamLIS as reported.
